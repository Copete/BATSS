...
for i=0,n_src-1 do begin
   ...
   for j=0,2 do begin
      ...
      plothist, lc.cent_snr[max_eband], title=src+': '+lc_type+' detection S/N ('+max_eband_str+')'
      oplot, snr_max*[1,1], [0,700], linestyle=2
      ;legend, 'Sigma = '+string(stddev(lc.cent_snr[max_eband]),f='(f5.3)')
   endfor
   line += (i lt n_src-1 ? ' \\' : '')
   table = [table, line]
   device, /close_file
